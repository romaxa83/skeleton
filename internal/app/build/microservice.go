...
func CreateMicroservice(config *config.MicroserviceConfig) {

	//fmt.Println(config.FrontFramework)
...
func createDockerCompose(config *config.MicroserviceConfig, path string, pathToDockerCompose string)  {
	...
	servicePhp := php.Create(config.Php, path,config.ProjectName.Name)
	...
	console.Info("DOCKER-COMPOSE CREATED")
